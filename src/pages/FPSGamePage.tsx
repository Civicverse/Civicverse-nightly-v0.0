...
interface Enemy {
  ...
  mesh: THREE.Group;
  health: number;
  type: 'street-thug' | 'enforcer' | 'rival';
  aiState: {
    lastShotTime: number;
...
export function FPSGamePage() {
  ...
  useEffect(() => {
    if (!gameContainerRef.current || !gameState.gameActive) return;
    ...
    const camera = new THREE.PerspectiveCamera(
      75,
      gameContainerRef.current.clientWidth / gameContainerRef.current.clientHeight,
      0.1,
      1000
    );
    ...
    const createCar = () => {
      ...
      carPhysicsBody.position.copy(carGroup.position as any);
    ...
    renderer.domElement.addEventListener('click', () => {
      renderer.domElement.requestPointerLock =
        renderer.domElement.requestPointerLock || (renderer.domElement as any).mozRequestPointerLock;
      renderer.domElement.requestPointerLock?.();
    });
    ...
    const animate = () => {
      ...
      camera.position.lerp(
        new THREE.Vector3(
          playerBody.position.x + cameraOffset.x,
          playerBody.position.y + cameraOffset.y,
          playerBody.position.z + cameraOffset.z
        ),
        0.1
      );
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col overflow-hidden">
      <div className="bg-gradient-to-r from-black via-gray-900 to-black border-b-4 border-cyan-500 p-4 flex justify-between items-center">
        <motion.button
          whileHover={{ scale: 1.05 }}
          onClick={() => navigate('/foyer')}
          className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 font-black text-lg"
        >
        ...
        <div className="text-center">
          <h1 className="text-2xl font-black text-cyan-400 tracking-wider">NEON CITY COMBAT</h1>
          <p className="text-xs text-gray-400 font-mono">STREET WARFARE • P2P BETTING • SYNTHWAVE</p>
        </div>

        <div className="text-right">
          <div className="text-cyan-400 font-black">
            BALANCE: {wallet?.balance?.toFixed(1)} CIVIC
          </div>
          <div className="text-xs text-gray-400">WALLET STATUS: ACTIVE</div>
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-4 p-4 overflow-hidden">
        <div className="flex-1 min-w-0 flex flex-col">
          <div
            ...
            className="relative flex-1 bg-black border-4 border-cyan-500 rounded-lg overflow-hidden"
          >
            {gameState.gameActive && (
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">
                  <div className="w-8 h-8 border-2 border-cyan-400">
                    <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1 w-0.5 h-2 bg-cyan-400" />
                    <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1 w-0.5 h-2 bg-cyan-400" />
                    <div className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1 w-2 h-0.5 bg-cyan-400" />
                    <div className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1 w-2 h-0.5 bg-cyan-400" />
                  </div>
                </div>

                <div className="absolute top-4 left-4 right-4 flex justify-between text-cyan-400 font-mono text-sm">
                  <div className="border-2 border-cyan-400 px-3 py-2 bg-black/50">
                    <div className="text-xs text-gray-400">HEALTH</div>
                    <div className="text-lg font-black">{gameState.health}%</div>
                    <div className="w-32 h-2 bg-gray-800 border border-cyan-400 mt-1">
                      <div
                  ...
                  <div className="border-2 border-cyan-400 px-3 py-2 bg-black/50 text-center">
                    <div className="text-xs text-gray-400">KILLS</div>
                    <div className="text-2xl font-black">{gameState.kills}</div>
                  </div>

                  <div className="border-2 border-cyan-400 px-3 py-2 bg-black/50">
                    <div className="text-xs text-gray-400">AMMO</div>
                    <div className="text-lg font-black">{gameState.ammo}</div>
                  </div>
                </div>

                <div className="absolute bottom-4 left-4 right-4 border-2 border-cyan-400 px-4 py-3 bg-black/50 text-cyan-400 font-mono text-xs">
                  <div className="flex justify-between">
                    <div>
                      <span className="text-gray-400">BET:</span> {gameState.bettedAmount} CIVIC
                    </div>
                    <div>
                      <span className="text-gray-400">POT:</span> {gameState.bettedAmount * 2} CIVIC
                    </div>
                    <div>
                      <span className="text-gray-400">POTENTIAL WIN:</span> +{gameState.kills * gameState.bettedAmount} CIVIC
                    </div>
                  </div>
            ...
            {!gameState.gameActive && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/80 backdrop-blur">
                <div className="text-center">
                  <div className="text-4xl font-black text-cyan-400 mb-4">ENTER THE STREETS</div>
                  <div className="text-gray-400 mb-8">Place your bet and dominate the neon city...</div>
                </div>
              </div>
            )}
          </div>

          <div className="mt-4 bg-gradient-to-r from-black via-gray-900 to-black border-4 border-cyan-500 p-3 flex justify-around text-cyan-400 font-mono text-sm">
            <div>
              <span className="text-gray-400">KILLS:</span> {gameState.kills}
            </div>
            <div>
              <span className="text-gray-400">HEALTH:</span> {gameState.health}%
            </div>
            <div>
              <span className="text-gray-400">AMMO:</span> {gameState.ammo}
            </div>
            <div>
              <span className="text-gray-400">BET:</span> {gameState.bettedAmount} CIVIC
            </div>
        ...
        <div className="w-80 flex flex-col bg-gradient-to-b from-gray-900 via-black to-gray-900 border-4 border-cyan-500 rounded-lg overflow-hidden">
          <div className="bg-black border-b-4 border-cyan-500 p-3">
            <h2 className="text-cyan-400 font-black text-sm flex items-center gap-2">
              <Users className="w-4 h-4" /> STREET NETWORK
            </h2>
            <p className="text-xs text-gray-400 mt-1">LIVE CITY CHAT</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 min-h-0">
            {chatMessages.map((msg) => (
              <motion.div
                ...
                className="border-l-2 border-cyan-400 pl-2 py-1"
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xl">{msg.avatar}</span>
                  <span className="text-cyan-400 font-mono text-xs font-bold">{msg.author}</span>
          ...
          <div className="bg-black border-t-4 border-cyan-500 p-3">
            <div className="flex gap-2">
              <input
                ...
                placeholder="Message..."
                className="flex-1 bg-gray-900 border-2 border-cyan-500 text-cyan-400 text-xs px-2 py-1 rounded focus:outline-none focus:border-cyan-300 placeholder-gray-600"
              />
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={handleSendMessage}
                className="bg-cyan-500 hover:bg-cyan-400 text-black p-2 rounded transition font-bold"
              >
          ...
          <div className="bg-black border-t-4 border-cyan-500 p-3 text-xs">
            <h3 className="text-cyan-400 font-black mb-2">STREET LEGENDS</h3>
            <div className="space-y-1">
              {[
                ...
                <div key={player.name} className="flex justify-between text-gray-400 border-b border-gray-800 pb-1">
                  <span className="text-cyan-400 font-mono">{player.name}</span>
                  <span>{player.kills} kills • {player.bet}₵</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <AnimatePresence>
        {showBetModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/80 backdrop-blur flex items-center justify-center z-50"
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-gradient-to-b from-gray-900 to-black border-4 border-cyan-500 p-8 rounded-lg max-w-md w-full"
            >
              <h2 className="text-2xl font-black text-cyan-400 mb-4">PLACE YOUR BET</h2>

              <div className="mb-6 p-4 border-2 border-cyan-500 bg-black/50 rounded">
                <div className="text-gray-400 text-sm mb-2">AVAILABLE BALANCE</div>
                <div className="text-2xl font-black text-cyan-400">{wallet?.balance?.toFixed(1)} CIVIC</div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                {[50, 100, 250, 500].map((amount) => (
                  <motion.button
                    ...
                    className={`w-full p-3 border-2 rounded transition font-bold ${
                      selectedBet === amount
                        ? 'border-cyan-400 bg-cyan-500/20 text-cyan-400'
                        : 'border-gray-600 bg-gray-900 text-gray-400 hover:border-cyan-500'
                    }`}
                  >
                    BET {amount} CIVIC
                  </motion.button>
              ...
              <div className="grid grid-cols-2 gap-2 mb-6 text-xs text-gray-400 font-mono">
                <div className="border border-gray-600 p-2 rounded">
                  ...
                  <div className="text-cyan-400 font-bold">{selectedBet}</div>
                </div>
                <div className="border border-gray-600 p-2 rounded">
                  <div>IF YOU GET 10 KILLS</div>
                  <div className="text-cyan-400 font-bold">+{selectedBet * 10}</div>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={startGame}
                className="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-black py-4 rounded-lg transition text-lg"
              >
                ENTER THE STREETS
              </motion.button>

              <div className="mt-4 text-xs text-gray-500 text-center">
                WINNER TAKES ALL • NO RESPAWNS • DOMINATE THE CITY
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {showGameOver && gameOverData && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-gradient-to-b from-gray-900 to-black border-4 border-cyan-500 p-8 rounded-lg max-w-md w-full text-center"
            >
              <div className="text-3xl mb-4">
                {gameOverData.kills > 0 ? '🏆' : '💀'}
              </div>

              <h2 className="text-2xl font-black text-cyan-400 mb-6">
                {gameOverData.kills > 0 ? 'STREET VICTORY' : 'DEFEATED'}
              </h2>

              <div className="space-y-4 mb-8">
                <div className="border-2 border-cyan-500 p-3 bg-black/50 rounded">
                  <div className="text-gray-400 text-sm">KILLS</div>
                  <div className="text-3xl font-black text-cyan-400">{gameOverData.kills}</div>
                </div>

                <div className="border-2 border-cyan-500 p-3 bg-black/50 rounded">
                  <div className="text-gray-400 text-sm">BET AMOUNT</div>
                  <div className="text-2xl font-black text-cyan-400">{gameOverData.betAmount} CIVIC</div>
                </div>

                <div className="border-4 border-yellow-500 p-4 bg-yellow-500/10 rounded">
                  <div className="text-gray-400 text-sm">TOTAL WINNINGS</div>
                  <div className="text-4xl font-black text-yellow-400">+{gameOverData.winnings} CIVIC</div>
                </div>
              </div>
              ...
              <motion.button
                ...
                className="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-black py-3 rounded-lg transition mb-2"
              >
                PLAY AGAIN
              </motion.button>
